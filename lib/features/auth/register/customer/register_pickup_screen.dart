// lib/features/auth/register/customer/register_pickup_screen.dart
...
class RegisterPickupScreen extends StatelessWidget {
  final _formKey = GlobalKey<FormState>();
  final _phoneCtrl = TextEditingController();
  final _nameCtrl = TextEditingController();
  final _cnicCtrl = TextEditingController();

  RegisterPickupScreen({super.key});
              ...
              child: Column(
                children: [
                  ...
                  AppTextField(
                    ...
                    validator: (v) {
                      if (v == null || v.trim().isEmpty) return 'شناختی کارڈ درکار ہے';
                      final clean = v.replaceAll('-', '');
                      if (clean.length != 13) return 'شناختی کارڈ 13 ہندسوں کا ہونا چاہیے';
                      return null;
                    },
                  ),
                          ...
                          : () {
                              if (_formKey.currentState!.validate()) {
                                context.read<AuthBloc>().add(
                                  AuthEvent.registerCustomer(
                                    phone: _phoneCtrl.text.trim(),
                                    name: _nameCtrl.text.trim(),
                                    cnic: _cnicCtrl.text.trim().replaceAll('-', ''),
                                  ),
                                );
                              }
                            },
